...
class CalculateClinicalMetrics {
    ...
    calculateVisceralFat(bmi, age, gender) {
    ...
    calculateMuscleMass(weightKg, heightCm, gender) {
    ...
    calculateBiologicalAge(age, bmi, gender) {
        let bioAge = age;
        if (bmi > 30)
            bioAge += (bmi - 30) * 0.5;
        if (bmi < 18.5)
    ...
    calculateRestingMetabolism(weightKg, heightCm, age, gender) {
        if (gender === "male") {
            return 88.362 + 13.397 * weightKg + 4.799 * heightCm - 5.677 * age;
        }
        return 447.593 + 9.247 * weightKg + 3.098 * heightCm - 4.330 * age;
    }
    calculateRiskLevel(bmi, bodyFatPercentage) {
        if (bmi >= 40 || bodyFatPercentage > 40)
            return "Muy alto";
        if (bmi >= 30 || bodyFatPercentage > 30)
            return "Alto";
        if (bmi >= 25 || bodyFatPercentage > 25)
            return "Moderado";
        if (bmi < 18.5)
            return "Moderado";
        return "Normal";
